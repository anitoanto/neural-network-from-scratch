...
def trainmodel(modeldata, nepoch, nbatch, resume=False):
    ...
    if resume == True:
        resumeFile = open("ResumeTrainData/ResumeData.txt")
        resumedata = resumeFile.readlines()[0].split(":")
        resumeFile.close()
    ...
    for e in range(nepoch):
        ...
        if acc > bestacc:
            ...
            trainedModelFile = open("TrainedModelData/ModelData.txt", "w")
            trainedModelFile.write(data)
        ...
        resumeTrainFile = open("ResumeTrainData/ResumeData.txt", "w")
        resumeTrainFile.write(data)
        resumeTrainFile.close()

        print(
            "Epoch : " + str(e + 1 + startepoch) + "  |  Accuracy : " + str(acc) + "%"
        )


def FASTtrainmodel(modeldata, nepoch, nbatch, resume=False):
    # does not test model in each epoch, saves only after defined nepoch
    x = np.load("TrainingData.npy")
    y = np.load("TrainingLabel.npy")
    testx = np.load("TestData.npy")
    testy = np.load("TestLabel.npy")
    ...
    startepoch = 0
    if resume == True:
        resumeFile = open("FastSaves/Details.txt")
        resumedata = resumeFile.readlines()[0].split(":")
        resumeFile.close()
        startepoch = int(resumedata[1])
    ...
    fastSaveFile = open("FastSaves/Details.txt", "w")
    fastSaveFile.write(data)
    fastSaveFile.close()
